...
static void update_proc(Layer* layer, GContext* ctx) {
  ...
  // get number
  int percent = s_charge_state.charge_percent;
  GPoint p = {
  ...
  if (percent == 100) {
    ...
    // Draw tens digit
    int tens = percent / 10;
    graphics_draw_tiny_number(ctx, tens, p.x, p.y);
    // Draw ones digit
    int ones = percent % 10;
    graphics_draw_tiny_number(ctx, ones, p.x + TN_WIDTH + 1, p.y);
  }
}
...
void watch_battery_layer_create() {
  ...
  if (!s_layer) {
    GRect rect = gbitmap_get_bounds(s_bitmap_battery);
    rect.size.w += MARGIN + (TN_WIDTH * 2) + 1; // Space for two digits with 1px spacing
